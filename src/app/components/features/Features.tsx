...
const features = [
  ...
  const FeatureCard = ({ icon, title, description }: Feature) => (
    <motion.div
      className="bg-white min-h-60 p-6 rounded-lg shadow-lg hover:shadow-xl transition-shadow duration-300 ease-in-out"
      whileHover={{ y: -5 }}
    >
      {icon}
  ...
  export default function Features() {
    return (
      <section className="py-20 relative overflow-hidden my-10 px-10">
        <div className="absolute inset-0 bg-inherit z-0"></div>
        <div className="absolute inset-0 z-10"></div>
        <div className="container mx-auto relative z-20">
          <motion.h2
            className="text-4xl md:text-5xl mb-20 font-bold text-center text-gray-900 dark:text-white"
          ...
          >
            <Button
              size="lg"
              className="bg-gradient-to-r from-primary to-primary-600 hover:from-primary-600 hover:to-primary-700 text-white font-semibold py-3 px-8 rounded-full transition-all duration-300 ease-in-out transform hover:scale-105 hover:shadow-lg"
            >
